...
var (
	...
	// Internal state / lifecycle errors (formerly panics).
	// These replace panic(...) calls on the Update/ReceiveMessage hot path so a
	// malformed message or a transiently invalid internal state never crashes
	// the whole embedder (orderer) process.
	ErrRoundChangeTimeoutNotSet = errors.New("stageRoundChanging entered without rcTimeout set")
	ErrLockTimeoutNotSet        = errors.New("stageLock entered without lockTimeout set")
	ErrCommitTimeoutNotSet      = errors.New("stageCommit entered without commitTimeout set")
	ErrLockReleaseTimeoutNotSet = errors.New("stageLockRelease entered without lockReleaseTimeout set")
	ErrMarshalOutgoingMessage   = errors.New("failed to marshal outgoing signed message")
	ErrMarshalResyncProof       = errors.New("failed to marshal proof in <resync> message")

	// Reconfiguration (PR-A3) — UpdateParticipants must be called between
	// heights. This error is returned when there is still state for the
	// in-flight height (non-empty roundchanges, commits, locks, or a
	// round-number > 0).
	ErrUpdateParticipantsHeightInFlight = errors.New("UpdateParticipants called while a height is in flight")
)
